...
class Heap {
 ...
 public:
  ...
  void Update(int address, int val) { mData[address] = val; }
  int get(int address) {
 ...
 public:
  ...
  void binop(BinaryOperator *bop) {
    ...
    if (bop->isAssignmentOp()) {
      // cout<< "  binop = " << bop << endl;
      // cout << "left " << left << " " << "left type " <<
      // (left->getType().getAsString()) << endl; cout << "right " << right << "
      // " << "right type " << (right->getType().getAsString()) << endl;
      int val = mStack.back().getStmtVal(right);
      if (DeclRefExpr *declexpr = dyn_cast<DeclRefExpr>(left)) {
        // cout << "binop declrefexpr " << endl;
        Decl *decl = declexpr->getFoundDecl();
        val = mStack.back().getStmtVal(right);
        // cout << "binop declrefexpr "<<
        // (declexpr->getFoundDecl()->getNameAsString()) << " " << decl << "  "
        // << val << endl;
    ...
    } else if (bop->isAdditiveOp() || bop->isMultiplicativeOp() ||
               bop->isComparisonOp()) {
      // left&right value
  ...
  void decl(DeclStmt *declstmt) {
    for (DeclStmt::decl_iterator it = declstmt->decl_begin(),
      ...
      if (VarDecl *vardecl = dyn_cast<VarDecl>(decl)) {  // if decl is vardecl
        // cout << "declstmt is vardecl " <<
        // (vardecl->getDefinition()->getNameAsString()) << " " << vardecl <<
        // endl;
        if (vardecl->hasInit()) {
          ...
          if (IntegerLiteral *integer = dyn_cast<IntegerLiteral>(expr)) {
            ...
            mStack.back().bindDecl(vardecl,
                                   val);  // binder(refers next statement)
          }
  ...
  void declref(DeclRefExpr *declref) {
    ...
    if (qualType->isPointerType() || qualType->isArrayType()) {
      ...
      // cout<< "integer  declref " <<
      // (declref->getFoundDecl()->getNameAsString()) << " " << declref << endl;
      int val = mStack.back().getDeclVal(decl);
      mStack.back().bindStmt(declref, val);
    }
  }
  ...
  void implicitCastExpr(ImplicitCastExpr *implicitCastExpr) {
    ...
    if (castKind == CK_LValueToRValue) {
      ...
      if (declRef) {
        ...
        value = mStack.back().getStmtVal(castedExpr);
        // cout << "CK_LValueToRValue array " << castedExpr << " val " << value
        // << endl;
        int val = mHeap.get(value);
        ...
        mStack.back().bindStmt(implicitCastExpr, val);
               ...
               castKind == CK_ArrayToPointerDecay || castKind == CK_BitCast) {
  ...
  void unaryExprOrTypeTraitExpr(UnaryExprOrTypeTraitExpr *uettExpr) {
    ...
    if ((uettExpr->getArgumentType().getTypePtr())->isIntegerType() ||
        (uettExpr->getArgumentType().getTypePtr())->isPointerType()) {
